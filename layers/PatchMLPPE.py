...
def MLPMixer(channels, patch_size, num_patches, d_model):
    ...
    return nn.Sequential(
        ...
        # nn.Linear(num_patches, (patch_size * num_patches) * channels),
        # nn.Linear((patch_size * num_patches) * channels, dim),
        nn.Linear(num_patches + 1, dim),
        # Reduce('b n c d-> b n d', 'mean'),
        *[
            nn.Sequential(
                PreNormResidual(
                    dim, FeedForward(num_patches, expansion_factor, dropout, chan_last)
                ),  # chan_first,num_patches
                PreNormResidual(
                    dim, FeedForward(dim, expansion_factor_token, dropout, chan_last)
                ),
            )
            for _ in range(depth)
        ],
        nn.LayerNorm(dim),
        Rearrange("b a n c -> b a c n"),
...
class PatchTST_backbone(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.backbone = MLPMixer(c_in, patch_len, patch_num, d_model)
        self.pos_embedding = nn.Parameter(
            torch.randn(1, patch_num + 1, patch_len, c_in)
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, patch_len, c_in))
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        z = z.permute(0, 2, 3, 1)  # [bs x  patch_num x patch_len x nvars]
        bs, patch_num, patch_len, nvars = z.shape
        cls_tokens = repeat(self.cls_token, "1 1 p d -> b 1 p d", b=bs)
        z = torch.cat((cls_tokens, z), dim=1)
        z += self.pos_embedding  #  [bs x patch_num+1 x patch_len x nvars]
        z = z.permute(0, 3, 2, 1)  # z: [bs x nvars x patch_len x (patch_num+1)]
        # model
